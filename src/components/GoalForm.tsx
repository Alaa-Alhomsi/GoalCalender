...
export function GoalForm({ onSubmit, initialGoal, isEditing = false }: GoalFormProps) {
  const { t } = useTranslation();
  const { addToast } = useToast();
  const [name, setName] = useState(initialGoal?.name || '');
  const [duration, setDuration] = useState(initialGoal?.duration || 365);
  const [unit, setUnit] = useState<TimeUnit>(initialGoal?.unit || 'days');
  const [startDate, setStartDate] = useState(formatDate(initialGoal?.startDate || new Date()));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addToast(
      'success',
      isEditing ? t('goal_updated_successfully') : t('goal_created_successfully')
    );
    ...
    <form className="goal-form" onSubmit={handleSubmit}>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          ...
          <select id="unit" value={unit} onChange={(e) => setUnit(e.target.value as TimeUnit)}>
